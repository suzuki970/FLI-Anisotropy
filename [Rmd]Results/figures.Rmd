---
title: "Figures' note for FLI-Anistropy"
author: "Yuta Suzuki"
...
Yuta Suzuki$^1$, Sumeyya Atmaca$^2$, Bruno Laeng$^2$
...
p <- ggplot(tmp_plot,aes(x=data_x, y=PDR, group=interaction(sub)),color="blue")+
  ...
  geom_point(data=minLatency,aes(x=min,y=ave_min_y),alpha=0.5,size=2,color="red")+
  geom_point(data=minLatency,aes(x=max,y=ave_max_y),alpha=0.5,size=2,color="blue")+
  xlab("Time [sec]")+
...
cormat_melt <- cor(tmp_plot, method = "spearman") %>% melt()
...
p_all$corrMat = ggplot(data = cormat_melt, aes(x = X1, y = X2, fill = value)) + 
  # ggplot(data = cormat_melt, aes(x = Var1, y = Var2, fill = value)) + 
  geom_tile() +
  geom_text(aes(label = round(value, 2)), position=position_nudge(y=0.2), size=0.5, colour="grey20") +
